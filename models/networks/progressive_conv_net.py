...
class GNet(nn.Module):
    ...
    def __init__(self,
        ...
        Args:
            ...
            - dimOutput (int): dimension of the output image. 3 -> RGB, 1 ->
                               grey levels
            - equalizedlR (bool): set to true to initiualize the layers with N(0,1) and
                                  apply He's constant at runtime
    ...
    def initFormatLayer(self, dimLatentVector):
        ...
        self.formatLayer = EqualizedLinear(self.dimLatent, 16 * self.scalesDepth[0],
...
class DNet(nn.Module):
    ...
    def __init__(self,
        ...
        Args:
            ...
            - decisionActivation: activation function of the decision layer. If
                                  ...
                                  For the training stage, it's advised to set this
                                  parameter to None and handle the activation
                                  function in the loss criterion.
            - sizeDecisionLayer: size of the decision layer. Will typically be
                                 greater than 2 when ACGAN is involved
            - miniBatchNormalization: do we apply the mini-batch normalization at
                                    at the last scale ?
        ...
        self.groupScaleZero.append(EqualizedLinear(depthScale0 * 16, depthScale0,
                                                      equalized = equalizedlR,
                                                      initBiasToZero = initBiasToZero))
    ...
    def addScale(self, depthNewScale):
        ...
        self.scaleLayers[-1].append(EqualizedConv2d(depthNewScale, depthNewScale,
                                                    ...
                                                    equalized = self.equalizedlR,
                                                    initBiasToZero = self.initBiasToZero))
        self.scaleLayers[-1].append(EqualizedConv2d(depthNewScale, depthLastScale,
                                                    3, padding =1,
                                                    equalized = self.equalizedlR,
                                                    initBiasToZero = self.initBiasToZero))

        self.fromRGBLayers.append(EqualizedConv2d(self.dimInput, depthNewScale, 1,
                                                 equalized = self.equalizedlR,
                                                 initBiasToZero = self.initBiasToZero))
