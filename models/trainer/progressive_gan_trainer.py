...
class ProgressiveGANTrainer(GANTrainer):
    ...
    def readTrainConfig(self, config):
        ...
        if self.modelConfig.alphaJumpMode not in ["custom", "linear"]:
            raise ValueError("alphaJumpMode should be one of the followings: 'custom', 'linear'")
    ...
    def scaleSanityCheck(self):
        ...
        for scale in range(1, n_scales):
            self.modelConfig.size_scales.append(self.modelConfig.size_scales[-1] * 2)

        self.modelConfig.n_scales = n_scales

    def updateAlphaJumps(self, nJumpScale, sizeJumpScale):
        r"""
        Given the number of iterations between two updates of alpha at each scale
        and the number of updates per scale, build the effective values of
        self.maxIterAtScale and self.alphaJumpVals.

        Args:

            - nJumpScale (list of int): for each scale, the number of times alpha
                                        should be updated
            - sizeJumpScale (list of int): for each scale, the number of iterations
                                           between two updates
        """
    ...
    def train(self):
        ...
        for scale in range(self.startScale, n_scales):
            ...
            while scale >= len(self.lossProfile):
                self.lossProfile.append({"G": [], "D":[], "scale": scale, "iter":[]})
            ...
            # Save a checkpoint
            if self.checkPointDir is not None:
                realIter = min(shiftIter, self.modelConfig.maxIterAtScale[scale])
    ...
    def addNewScales(self, configNewScales):
        ...
        if configNewScales["alphaJumpMode"] == 'custom':
            self.modelConfig.iterAlphaJump = self.modelConfig.iterAlphaJump + configNewScales["iterAlphaJump"]
            self.modelConfig.alphaJumpVals = self.modelConfig.alphaJumpVals + configNewScales["alphaJumpVals"]

        else:
            self.updateAlphaJumps(configNewScales["alphaNJumps"], configNewScales["alphaSizeJumps"])

        self.modelConfig.depthScales = self.modelConfig.depthScales + configNewScales["depthScales"]
        self.modelConfig.maxIterAtScale = self.modelConfig.maxIterAtScale + configNewScales["maxIterAtScale"]
