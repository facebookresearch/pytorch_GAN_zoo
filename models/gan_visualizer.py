...
class GANVisualizer():
    ...
    def buildKeyShift(self):
        ...
        self.keyShift = { f : {} for f in self.model.config.attribKeysOrder.keys()}
    ...
    def exportVisualization(self,
        ...
        while remaining > 0:
            ...
            if export_mask:
                try:
                    _, shape, texture = self.model.getDetailledOutput(noiseData)
        ...
        self.visualizer.saveTensor(toSave, (toSave.size()[2], toSave.size()[3]), path)

        if len(outTexture) > 0:
            toSave = torch.cat(outTexture, dim = 0)
            pathTexture = os.path.splitext(path)[0] + "_texture.png"
            self.visualizer.saveTensor(toSave, (toSave.size()[2], toSave.size()[3]), pathTexture)

            toSave = torch.cat(outShape, dim = 0)
            pathShape = os.path.splitext(path)[0] + "_shape.png"
            self.visualizer.saveTensor(toSave, (toSave.size()[2], toSave.size()[3]), pathShape)
    ...
    def saveInterpolation(self, N, vectorStart, vectorEnd, pathOut):
        ...
        for i in range(N):
            ...
            self.visualizer.saveTensor(outImg, (outImg.size(2), outImg.size(3)), path)
    ...
    def visualizeNN(self, N, k , featureExtractor, imgTransform, nnSearch, names, pathDB):
        ...
        while nImages < N:
            ...
            imgOut = self.model.test(noiseData, getAvG = True, toCPU = False).detach()

            features = featureExtractor(imgTransform(imgOut)).detach().view(imgOut.size(0), -1).cpu().numpy()
            distances, indexes = nnSearch.query(features, k)
            nImages+= batchSize

            for p in range(N):

                vectorOut.append(imgOut[p].cpu().view(1, imgOut.size(1), imgOut.size(2), imgOut.size(3)))
                for ki in range(k):
                    ...
                    if dataset is None:
                        ...
                        imgSource = imgSource.view(1, imgSource.size(0), imgSource.size(1), imgSource.size(2))
