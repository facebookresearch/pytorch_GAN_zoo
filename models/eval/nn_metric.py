...
def test(parser, visualisation = None):
    ...
    imgTransform = nn.DataParallel(FeatureTransform(mean, std, kwargs['size'])).to(torch.device("cuda:0"))
    featureExtractor = nn.DataParallel(featureExtractor).to(torch.device("cuda:0"))

    with open(pathOutFeatures, 'rb') as file:
        nnSearch, names = pickle.load(file)

    if kwargs['showNN']:
        visualizer.visualizeNN(10, 5, featureExtractor, imgTransform, nnSearch, names, pathDB)
    else:

        outMetric = visualizer.exportNN(1600, 8, featureExtractor,imgTransform, nnSearch)
        outPath = modelConfig[:-18] + "_nn_metric.json"

        saveScore(outPath, list(outMetric), scale, iter, pathNNFeatureExtractor)
